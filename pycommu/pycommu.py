...
class PyCommu:
    ...
    def _send(
        ...
        if face_smile is not None:
            data["face_smile"] = face_smile
        if face_search is not None:
            data["face_search"] = face_search
        if age_and_sex is not None:
            data["age_and_sex"] = age_and_sex
        if name is not None:
            data["name"] = name
    ...
    def wait_for_speech_recognition(self, timeout=None):
        with self.__lock:
            while 1:
                self._send(
                    "get_recog_res", timeout=timeout if timeout is not None else 5000
                )
                res = self._read_data()
                print(res)
                res = res["string"]
